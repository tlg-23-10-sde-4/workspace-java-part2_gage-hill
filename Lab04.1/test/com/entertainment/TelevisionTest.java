...
public class TelevisionTest {
     ...
     * It also allows us to test multiple calls to the business method in one test method (which may not always be
     * desirable).
     * We are testing both lower and upper bounds in this one test method (mainly to show the technique, and that you
     * can).
     */
    @Test
    public void testSetVolumeIllegalArgumentException() {
        ...
        try {
            ...
            assertEquals("Invalid volume: -1. Allowed range: [0,100].", e.getMessage());  // verify the exception
            // message
        }
        ...
        try {
            ...
            assertEquals("Invalid volume: 101. Allowed range: [0,100].", e.getMessage());  // verify the exception
            // message
        }
    }

    @Test
    public void testChangeChannel() throws InvalidChannelException {  // throws clause needed because it's an
     ...
     * This is because once the exception is thrown, we don't catch it here, which means it propagates back to the
     * caller of
     * the test method (the JUnit test runner), and method execution terminates immediately - normal exception behavior.
     */
    @Test(expected = InvalidChannelException.class)
    public void testChangeChannelInvalidChannelException_LowerBound() throws InvalidChannelException {  // unhandled
        // checked exception
        Television tv = new Television();
        tv.changeChannel(0);
    ...
    @Test(expected = InvalidChannelException.class)
    public void testChangeChannelInvalidChannelException_UpperBound() throws InvalidChannelException {  // unhandled
        // checked exception
        Television tv = new Television();
